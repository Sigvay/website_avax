...
def export_avito(data):
    ...
    for result in results.json():
        ...
        ET.SubElement(ad, "Description").text = etr.CDATA(str(
            ...
            f"Двигатель: {result['ОБЪЕМ']}{result['ТИП ДВИГАТЕЛЯ']}  {result['МАРКИРОВКА ДВИГАТЕЛЯ']} Б/У.<br /><br />" +
            "ЦЕНА УКАЗАНА ЗА ДВИГАТЕЛЬ В СБОРЕ, В КОМПЛЕКТАЦИИ КАК НА ФОТО!<br />"+
            f"У нас в наличии контрактный двигатель {result['МАРКА']} {result['МОДЕЛЬ']} {result['ОБЪЕМ']}{result['ТИП ДВИГАТЕЛЯ']}  {result['МАРКИРОВКА ДВИГАТЕЛЯ']}, произведенный в {result['ГОД']} году."+
            "Данный двигатель в отличном состоянии и готов к установке. Он был снят с рабочего машинокомплекта и имеет небольшой пробег. Двигатель не имеет пробега по СНГ. <br />"+
            "Мы предлагаем качественные б/у двигатели по низким ценам, так как являемся прямым импортером. Компания AVAX Motors гарантирует высокое качество товара и отличное обслуживание. <br />"+
            "Вы можете связаться с нами по следующим контактам:"+
            "Адрес: Беларусь, Минская область, Узденский район, Озерский сельсовет, агрогородок Озеро, Институтский переулок, 2/2"+
            "Телефоны: +375 (29) 675-44-36, +375 (29) 311-83-35, +375 (29) 675 44 36<br /><br />"+
            "<strong>Данный ДВИГАТЕЛЬ находится на складе в Республике Беларусь</strong><br />"+
            "<strong>Если вы находитесь в Москве, срок поставки на наш склад займет всего 1-2 дня! Мы также предлагаем услуги доставки с помощью транспортных компаний.</strong><br /><br />"+
        ...
        ET.SubElement(ad, "Price").text = str((int(result["ЦЕНА"]) + 250) * data.exchange)
